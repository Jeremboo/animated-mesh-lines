...
export default (cameraAmpl = { x: 5, y: 5 }, velocity = 0.1, lookAt = new Vector3()) =>
  (Target) => class CameraPositionHandlerWithMouse extends Target {
    constructor(props) {
      super(props);
      ...
      this.normalizedMousePosition = new Vector3();
    ...
    update() {
      ...
      this.camera.position.x += (this.normalizedMousePosition.x - this.camera.position.x) * this.cameraVelocity;
      this.camera.position.y += (this.normalizedMousePosition.y - this.camera.position.y) * this.cameraVelocity;
